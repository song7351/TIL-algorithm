...
"""
원리:
() = L(레이저)로 바꿔서 새로운 리스트를 만듦
모든 막대기는 최소 한번의 레이저가 지나간다 = 전체 막대기의 최소 횟수 = 모든 막대기의 개수
모든 막대기의 개수 = '('만 카운트
L(레이저) 앞에 "("를 카운트 한다. = why? 그래야 해당 막대기가 지나감을 알수있다.
그러나 ")"는 빼준다 = why? 그래야 해당 막대기가 레이저를 지나기전에 끊긴다는것을 알수있다.
"""
test_case = int(input())
